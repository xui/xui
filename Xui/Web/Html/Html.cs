...
public ref struct Html
{
    ...
    public bool AppendFormatted(Action<Events.Subsets.Data> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Data.Format, expression);
    ...
    public bool AppendFormatted(Action<Events.Subsets.IsComposing> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.IsComposing.Format, expression);
    ...
    public bool AppendFormatted(Action<Events.Subsets.Buttons> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Buttons.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.Coordinates> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Coordinates.Format, expression);
    ...
    public bool AppendFormatted(Action<Events.Subsets.X> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.X.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.Y> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Y.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.ClientXY> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.ClientXY.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.MovementXY> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.MovementXY.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.OffsetXY> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.OffsetXY.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.PageXY> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.PageXY.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.ScreenXY> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.ScreenXY.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.Deltas> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Deltas.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.Touches> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Touches.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.RelatedTarget> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.RelatedTarget.Format, expression);
    public bool AppendFormatted(Action<Events.Subsets.Modifiers> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Modifiers.Format, expression);
    ...
    public bool AppendFormatted(Func<Events.Subsets.Data, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Data.Format, expression);
    ...
    public bool AppendFormatted(Func<Events.Subsets.IsComposing, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.IsComposing.Format, expression);
    ...
    public bool AppendFormatted(Func<Events.Subsets.Buttons, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Buttons.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.Coordinates, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Coordinates.Format, expression);
    ...
    public bool AppendFormatted(Func<Events.Subsets.X, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.X.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.Y, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Y.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.ClientXY, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.ClientXY.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.MovementXY, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.MovementXY.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.OffsetXY, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.OffsetXY.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.PageXY, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.PageXY.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.ScreenXY, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.ScreenXY.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.Deltas, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Deltas.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.Touches, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Touches.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.RelatedTarget, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.RelatedTarget.Format, expression);
    public bool AppendFormatted(Func<Events.Subsets.Modifiers, Task> eventHandler, string? format = null, [CallerArgumentExpression(nameof(eventHandler))] string? expression = null) => AppendEventHandler(eventHandler, format ?? Events.Subsets.Modifiers.Format, expression);
